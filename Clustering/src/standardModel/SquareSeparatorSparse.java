...
public class SquareSeparatorSparse implements SeparatorInterface
{
	...
	public void separate() throws IloException
	{
		...
		// Create variables
		double lowerLhs = _spanRhs ? 1 : 0;
		double upperLhs = _spanRhs ? 1 : 10 * _instance.getPoints();
		
		alpha = new IloNumVar[_instance.getPoints()];
		beta = cplex.numVar(-1e10, 1e10, "beta");
		a1 = cplex.numVar(lowerLhs, upperLhs, "a1");
		b1 = cplex.numVar(lowerLhs, upperLhs, "b1");
		a2 = cplex.numVar(lowerLhs, upperLhs, "a2");
		b2 = cplex.numVar(lowerLhs, upperLhs, "b2");
